...
class WP_Plugin_Filters_Health_Calculator {
	...
	public function get_health_description( $score ) {
		if ( $score >= 86 ) {
			return __( 'Excellent - Well maintained and actively supported', 'wp-plugin-filters' );
		} elseif ( $score >= 71 ) {
			return __( 'Good - Regularly maintained with good support', 'wp-plugin-filters' );
		} elseif ( $score >= 41 ) {
			return __( 'Fair - Occasionally maintained, some concerns', 'wp-plugin-filters' );
		} else {
			return __( 'Poor - Infrequently maintained, potential issues', 'wp-plugin-filters' );
		}
	}
	...
	public function get_algorithm_explanation() {
		return array(
			'title'        => __( 'Health Score Algorithm', 'wp-plugin-filters' ),
			'description'  => __( 'The health score evaluates plugin maintenance quality and reliability indicators.', 'wp-plugin-filters' ),
			'components'   => array(
				'update_frequency'  => array(
					'label'       => __( 'Update Frequency', 'wp-plugin-filters' ),
					'description' => __( 'How regularly the plugin is updated and maintained', 'wp-plugin-filters' ),
					'weight'      => $this->weights['update_frequency'],
				),
				'wp_compatibility'  => array(
					'label'       => __( 'WordPress Compatibility', 'wp-plugin-filters' ),
					'description' => __( 'Compatibility with current WordPress versions', 'wp-plugin-filters' ),
					'weight'      => $this->weights['wp_compatibility'],
				),
				'support_response'  => array(
					'label'       => __( 'Support Response', 'wp-plugin-filters' ),
					'description' => __( 'Developer responsiveness to support requests', 'wp-plugin-filters' ),
					'weight'      => $this->weights['support_response'],
				),
				'time_since_update' => array(
					'label'       => __( 'Update Recency', 'wp-plugin-filters' ),
					'description' => __( 'How recently the plugin was last updated', 'wp-plugin-filters' ),
					'weight'      => $this->weights['time_since_update'],
				),
				'reported_issues'   => array(
					'label'       => __( 'Issue Indicators', 'wp-plugin-filters' ),
					'description' => __( 'Indicators of potential issues from user feedback', 'wp-plugin-filters' ),
					'weight'      => $this->weights['reported_issues'],
				),
			),
			'scale'        => __( 'Final score ranges from 0 to 100 (higher is better)', 'wp-plugin-filters' ),
			'color_coding' => array(
				'green'       => __( 'Excellent (86-100)', 'wp-plugin-filters' ),
				'light-green' => __( 'Good (71-85)', 'wp-plugin-filters' ),
				'orange'      => __( 'Fair (41-70)', 'wp-plugin-filters' ),
				'red'         => __( 'Poor (0-40)', 'wp-plugin-filters' ),
			),
		);
	}
