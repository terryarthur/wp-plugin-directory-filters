...
class WP_Plugin_Filters_Rating_Calculator {
    ...
    private function calculate_rating_count_component($plugin_data) {
        ...
        if ($rating_count >= 1000) return 1.0;    // Excellent credibility
        if ($rating_count >= 500) return 0.9;     // Very good credibility
        if ($rating_count >= 100) return 0.8;     // Good credibility
        if ($rating_count >= 50) return 0.7;      // Fair credibility
        if ($rating_count >= 20) return 0.6;      // Some credibility
        if ($rating_count >= 10) return 0.5;      // Limited credibility
        if ($rating_count >= 5) return 0.4;       // Low credibility
    ...
    private function calculate_installation_component($plugin_data) {
        ...
        if ($installs >= 5000000) return 1.0;     // Extremely popular (5M+)
        if ($installs >= 1000000) return 0.95;    // Very popular (1M-5M)
        if ($installs >= 500000) return 0.9;      // Popular (500K-1M)
        if ($installs >= 100000) return 0.8;      // Well-used (100K-500K)
        if ($installs >= 50000) return 0.7;       // Moderately used (50K-100K)
        if ($installs >= 10000) return 0.6;       // Some usage (10K-50K)
        if ($installs >= 5000) return 0.5;        // Limited usage (5K-10K)
        if ($installs >= 1000) return 0.4;        // Low usage (1K-5K)
        if ($installs >= 100) return 0.3;         // Very low usage (100-1K)
    ...
    private function calculate_support_component($plugin_data) {
        ...
        if ($resolution_rate >= 0.9) return 1.0;    // Excellent support (90%+)
        if ($resolution_rate >= 0.8) return 0.9;    // Very good support (80-89%)
        if ($resolution_rate >= 0.7) return 0.8;    // Good support (70-79%)
        if ($resolution_rate >= 0.6) return 0.7;    // Fair support (60-69%)
        if ($resolution_rate >= 0.5) return 0.6;    // Poor support (50-59%)
        if ($resolution_rate >= 0.4) return 0.5;    // Bad support (40-49%)
        if ($resolution_rate >= 0.3) return 0.4;    // Very bad support (30-39%)
    ...
    public function update_weights($weights) {
        ...
        foreach ($required_components as $component) {
            if (!isset($weights[$component]) || !is_numeric($weights[$component])) {
                return new WP_Error('missing_component', sprintf(__('Missing or invalid weight for %s', 'wp-plugin-filters'), $component));
            }
        }
        ...
        if (abs($total_weight - 100) > 1) {
            return new WP_Error('invalid_total', __('Algorithm weights must sum to 100%', 'wp-plugin-filters'));
        }
        
        // Validate individual weights (0-100)
        foreach ($weights as $component => $weight) {
            if ($weight < 0 || $weight > 100) {
                return new WP_Error('invalid_range', sprintf(__('Weight for %s must be between 0 and 100', 'wp-plugin-filters'), $component));
            }
        }
    ...
    public function get_algorithm_explanation() {
        return array(
            'title' => __('Usability Rating Algorithm', 'wp-plugin-filters'),
            'description' => __('The usability rating combines multiple factors to provide an overall assessment of plugin usability and quality.', 'wp-plugin-filters'),
            'components' => array(
                'user_rating' => array(
                    'label' => __('User Rating', 'wp-plugin-filters'),
                    'description' => __('Average rating given by users on WordPress.org', 'wp-plugin-filters'),
                    'weight' => $this->weights['user_rating']
                ),
                'rating_count' => array(
                    'label' => __('Rating Credibility', 'wp-plugin-filters'),
                    'description' => __('Number of ratings (more ratings = higher credibility)', 'wp-plugin-filters'),
                    'weight' => $this->weights['rating_count']
                ),
                'installation_count' => array(
                    'label' => __('Popularity', 'wp-plugin-filters'),
                    'description' => __('Number of active installations (popularity indicator)', 'wp-plugin-filters'),
                    'weight' => $this->weights['installation_count']
                ),
                'support_responsiveness' => array(
                    'label' => __('Support Quality', 'wp-plugin-filters'),
                    'description' => __('Percentage of resolved support threads', 'wp-plugin-filters'),
                    'weight' => $this->weights['support_responsiveness']
                )
            ),
            'scale' => __('Final rating is scaled from 1.0 to 5.0 stars', 'wp-plugin-filters')
        );
    }
