...
class WP_Plugin_Filters_API_Handler {
	...
	public function search_plugins( $search_term = '', $page = 1, $per_page = 24, $filters = array() ) {
		...
		error_log( '[WP Plugin Filters] API Request Args: ' . print_r( $request_args, true ) );
		...
		if ( isset( $processed_response['plugins'] ) && count( $processed_response['plugins'] ) > 0 ) {
			error_log( '[WP Plugin Filters] First processed plugin: ' . print_r( $processed_response['plugins'][0], true ) );
		}
	...
	private function make_api_request( $action, $request_args ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'api_error', __( 'Failed to connect to WordPress.org API', 'wp-plugin-filters' ) );
		}

		$response_code = wp_remote_retrieve_response_code( $response );
		if ( $response_code !== 200 ) {
			error_log( '[WP Plugin Filters] API Request HTTP Error: ' . $response_code );
			return new WP_Error( 'api_http_error', sprintf( __( 'API request failed with status code: %d', 'wp-plugin-filters' ), $response_code ) );
		}
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			...
			return new WP_Error( 'json_decode_error', __( 'Invalid JSON response from WordPress.org API', 'wp-plugin-filters' ) );
		}
	...
	private function sanitize_plugin_data( $plugin ) {
		return array(
			...
			'author'                   => sanitize_text_field( strip_tags( $plugin['author'] ?? '' ) ),
	...
	private function check_rate_limit() {
		...
		if ( $current_count >= self::RATE_LIMIT_PER_MINUTE ) {
			return new WP_Error( 'rate_limit_exceeded', __( 'API rate limit exceeded. Please try again in a minute.', 'wp-plugin-filters' ) );
		}
