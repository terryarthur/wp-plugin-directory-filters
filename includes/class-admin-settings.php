...
class WP_Plugin_Filters_Admin_Settings {
     ...
     * Settings page slug
     */
    const SETTINGS_PAGE_SLUG = 'wp-plugin-filters';
    ...
    public function register_settings() {
        ...
        add_settings_section(
            'cache_section',
            __('Cache Settings', 'wp-plugin-filters'),
        ...
        $cache_fields = array(
            'plugin_metadata' => __('Plugin Metadata Cache Duration (seconds)', 'wp-plugin-filters'),
            'calculated_ratings' => __('Calculated Ratings Cache Duration (seconds)', 'wp-plugin-filters'),
            'search_results' => __('Search Results Cache Duration (seconds)', 'wp-plugin-filters'),
            'api_responses' => __('API Responses Cache Duration (seconds)', 'wp-plugin-filters')
        );
        ...
        add_settings_field(
            'cache_management',
            __('Cache Management', 'wp-plugin-filters'),
    ...
    public function enqueue_admin_scripts($hook_suffix) {
        ...
        wp_enqueue_script(
            'wp-plugin-filters-admin-settings',
        ...
        wp_localize_script('wp-plugin-filters-admin-settings', 'wpPluginFiltersAdmin', array(
            ...
            'strings' => array(
                'cacheCleared' => __('Cache cleared successfully', 'wp-plugin-filters'),
                'cacheClearError' => __('Error clearing cache', 'wp-plugin-filters'),
                'weightValidationError' => __('Algorithm weights must sum to 100%', 'wp-plugin-filters'),
                'testingAlgorithm' => __('Testing algorithm...', 'wp-plugin-filters'),
                'algorithmTested' => __('Algorithm test completed', 'wp-plugin-filters')
            )
        ));
    ...
    public function render_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Plugin Directory Filters Settings', 'wp-plugin-filters'); ?></h1>
            ...
            <div class="wp-plugin-filters-admin-info">
                ...
                <div class="algorithm-explanations">
                    ...
                    <div class="algorithm-explanation">
                        ...
                        <ul>
                            <li><span class="health-badge health-badge-green">86-100</span> <?php esc_html_e('Excellent', 'wp-plugin-filters'); ?></li>
                            <li><span class="health-badge health-badge-light-green">71-85</span> <?php esc_html_e('Good', 'wp-plugin-filters'); ?></li>
                            <li><span class="health-badge health-badge-orange">41-70</span> <?php esc_html_e('Fair', 'wp-plugin-filters'); ?></li>
                            <li><span class="health-badge health-badge-red">0-40</span> <?php esc_html_e('Poor', 'wp-plugin-filters'); ?></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    public function render_network_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Plugin Directory Filters Network Settings', 'wp-plugin-filters'); ?></h1>
            <p><?php esc_html_e('Configure plugin directory filter settings for the entire network.', 'wp-plugin-filters'); ?></p>
            ...
            <form method="post" action="">
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row"><?php esc_html_e('Override Site Settings', 'wp-plugin-filters'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="override_site_settings" value="1" <?php checked(!empty($network_settings['override_site_settings'])); ?> />
                                <?php esc_html_e('Force network-wide settings (override individual site settings)', 'wp-plugin-filters'); ?>
                            </label>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><?php esc_html_e('Enable for Sites', 'wp-plugin-filters'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="enable_for_sites" value="1" <?php checked(!empty($network_settings['enable_for_sites']), true); ?> />
                                <?php esc_html_e('Enable plugin directory filters for all sites in the network', 'wp-plugin-filters'); ?>
                            </label>
                        </td>
                    </tr>
                </table>
                
                <?php submit_button(__('Update Network Settings', 'wp-plugin-filters')); ?>
            </form>
        </div>
        <?php
    }
    ...
    public function render_cache_section_description() {
        echo '<p>' . esc_html__('Configure cache durations and manage cached data.', 'wp-plugin-filters') . '</p>';
        ...
        if ($stats['total_transients'] > 0) {
            ...
            echo '<h4>' . esc_html__('Current Cache Statistics', 'wp-plugin-filters') . '</h4>';
            echo '<p>' . sprintf(
                esc_html__('Total cached items: %d | Total size: %s', 'wp-plugin-filters'),
                $stats['total_transients'],
    ...
    public function render_cache_duration_field($args) {
        ...
        $descriptions = array(
            'plugin_metadata' => __('How long to cache plugin information from WordPress.org (recommended: 86400 = 24 hours)', 'wp-plugin-filters'),
            'calculated_ratings' => __('How long to cache calculated usability ratings and health scores (recommended: 21600 = 6 hours)', 'wp-plugin-filters'),
            'search_results' => __('How long to cache search results (recommended: 3600 = 1 hour)', 'wp-plugin-filters'),
            'api_responses' => __('How long to cache raw API responses (recommended: 1800 = 30 minutes)', 'wp-plugin-filters')
        );
    ...
    public function render_cache_management_field() {
        ...
        echo '<button type="button" id="clear-all-cache" class="button button-secondary">' . esc_html__('Clear All Cache', 'wp-plugin-filters') . '</button> ';
        echo '<button type="button" id="clear-search-cache" class="button button-secondary">' . esc_html__('Clear Search Cache', 'wp-plugin-filters') . '</button> ';
        echo '<button type="button" id="clear-ratings-cache" class="button button-secondary">' . esc_html__('Clear Ratings Cache', 'wp-plugin-filters') . '</button>';
        echo '<p class="description">' . esc_html__('Clear cached data to force fresh API requests and recalculations.', 'wp-plugin-filters') . '</p>';
        echo '</div>';
    }
    ...
    private function handle_settings_update() {
        $settings = $this->sanitize_settings($_POST[self::SETTINGS_OPTION] ?? array());
        
        if (update_option(self::SETTINGS_OPTION, $settings)) {
            add_settings_error(
                self::SETTINGS_OPTION,
                'settings_updated',
                __('Settings saved successfully.', 'wp-plugin-filters'),
                'updated'
            );
        } else {
            add_settings_error(
                self::SETTINGS_OPTION,
                'settings_error',
                __('Error saving settings.', 'wp-plugin-filters'),
                'error'
            );
        }
    }
