...
class WP_Plugin_Filters_Security_Handler {
    ...
    public function validate_ajax_request($nonce_action, $required_capability = 'install_plugins') {
        ...
        if (!wp_doing_ajax()) {
            return new WP_Error('not_ajax', __('This endpoint only accepts AJAX requests', 'wp-plugin-filters'));
        }
        ...
        if (!current_user_can($required_capability)) {
            return new WP_Error('insufficient_permissions', __('You do not have permission to perform this action', 'wp-plugin-filters'));
        }
        
        // Additional security checks
        if (!$this->validate_referrer()) {
            return new WP_Error('invalid_referrer', __('Invalid request origin', 'wp-plugin-filters'));
        }
    ...
    public function validate_plugin_slug($slug) {
        ...
        if (empty($slug)) {
            return new WP_Error('invalid_slug', __('Plugin slug cannot be empty', 'wp-plugin-filters'));
        }
        
        if (strlen($slug) > 100) {
            return new WP_Error('slug_too_long', __('Plugin slug is too long', 'wp-plugin-filters'));
        }
        
        // Plugin slugs should only contain lowercase letters, numbers, and hyphens
        if (!preg_match('/^[a-z0-9\-]+$/', $slug)) {
            return new WP_Error('invalid_slug_format', __('Invalid plugin slug format', 'wp-plugin-filters'));
        }
    ...
    public function check_rate_limit($action = 'general', $limit = 60, $window = 60) {
        ...
        if ($current_count >= $limit) {
            return new WP_Error(
                'rate_limit_exceeded', 
                sprintf(
                    __('Rate limit exceeded. Maximum %d requests per %d seconds allowed.', 'wp-plugin-filters'),
                    $limit,
                    $window
                )
            );
