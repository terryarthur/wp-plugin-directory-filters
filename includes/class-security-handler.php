...
class WP_Plugin_Filters_Security_Handler {
	...
	public function validate_ajax_request( $nonce_action, $required_capability = 'install_plugins' ) {
		...
		if ( ! wp_doing_ajax() ) {
			return new WP_Error( 'not_ajax', __( 'This endpoint only accepts AJAX requests', 'wp-plugin-directory-filters' ) );
		}

		// Verify nonce
		$nonce_key = 'nonce';
		if ( ! isset( $_POST[ $nonce_key ] ) || ! wp_verify_nonce( $_POST[ $nonce_key ], $nonce_action ) ) {
			return new WP_Error( 'nonce_verification_failed', __( 'Security verification failed', 'wp-plugin-directory-filters' ) );
		}

		// Check user capabilities
		if ( ! current_user_can( $required_capability ) ) {
			return new WP_Error( 'insufficient_permissions', __( 'You do not have permission to perform this action', 'wp-plugin-directory-filters' ) );
		}

		// Additional security checks
		if ( ! $this->validate_referrer() ) {
			return new WP_Error( 'invalid_referrer', __( 'Invalid request origin', 'wp-plugin-directory-filters' ) );
		}
	...
	public function validate_plugin_slug( $slug ) {
		...
		if ( empty( $slug ) ) {
			return new WP_Error( 'invalid_slug', __( 'Plugin slug cannot be empty', 'wp-plugin-directory-filters' ) );
		}

		if ( strlen( $slug ) > 100 ) {
			return new WP_Error( 'slug_too_long', __( 'Plugin slug is too long', 'wp-plugin-directory-filters' ) );
		}

		// Plugin slugs should only contain lowercase letters, numbers, and hyphens
		if ( ! preg_match( '/^[a-z0-9\-]+$/', $slug ) ) {
			return new WP_Error( 'invalid_slug_format', __( 'Invalid plugin slug format', 'wp-plugin-directory-filters' ) );
		}
	...
	public function check_rate_limit( $action = 'general', $limit = 60, $window = 60 ) {
		...
		if ( $current_count >= $limit ) {
			return new WP_Error(
				'rate_limit_exceeded',
				sprintf(
					__( 'Rate limit exceeded. Maximum %1$d requests per %2$d seconds allowed.', 'wp-plugin-directory-filters' ),
					$limit,
					$window
				)
			);
	...
	public function validate_url( $url ) {
		...
		if ( ! filter_var( $url, FILTER_VALIDATE_URL ) || ! in_array( parse_url( $url, PHP_URL_SCHEME ), array( 'http', 'https' ), true ) ) {
			return false;
		}
